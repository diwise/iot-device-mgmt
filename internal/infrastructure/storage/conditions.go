...
func extractCoordsFromQuery(bounds string) Bounds {
...
func OffsetLimit(c *Condition, i ...int) (string, int, int) {
	offsetLimit := ""
	offset := 0
	limit := 10
...
func OrderByWithFallback(c *Condition, fallback string) string {
	orderBy := ""
...
func alarmConditionFromQuery(query alarmquery.Alarms) *Condition {
	condition := &Condition{
